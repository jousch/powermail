 ...
 *
 *   52: class jscalendar
 *   74:     function jscalendar($itemConfig, $calendarCSS, $lang='', $altLang='', $datetimeToolbocks = true)
 *  107:     function getItemJS()
 *  148:     function getJS()
 *  202:     function languageCheck(&$lang)
 *
 * TOTAL FUNCTIONS: 4
 * (This index is automatically created/updated by the extension "extdeveval")
 *
 */
class tx_powermail_jscalendar {
	function main() {
		return;
	}
}
...
class jscalendar
{
	...
	function getJS()
	{
		...
		// multiple init check - don't return js code if already set
		if($GLOBALS['JSCalendarInitFlag']) return '';
		...
		if($this->config['lang'] != 'en')
			$adLang = '<script type="text/javascript" src="' . $backPath .
				t3lib_extMgm::siteRelPath('date2cal') . 'jscalendar/lang/calendar-' .
		...
		$js = '
			...
			<script type="text/javascript" src="' . $backPath .
				t3lib_extMgm::siteRelPath('date2cal') . 'jscalendar/calendar.js"></script>
			<script type="text/javascript" src="' . $backPath .
				t3lib_extMgm::siteRelPath('date2cal') . 'jscalendar/lang/calendar-en.js"></script> ' .
				$adLang . '
			<script type="text/javascript" src="' . $backPath .
				t3lib_extMgm::siteRelPath('date2cal') . 'jscalendar/calendar-setup.js"></script>
			<link rel="stylesheet" type="text/css" href="' . $backPath .
				t3lib_extMgm::siteRelPath('date2cal') . 'jscalendar/skins/' .
	...
	function languageCheck(&$lang)
	{
		...
		if(isset($GLOBALS['LANG']->csConvObj->isoArray)) {
			if(array_key_exists($lang, $GLOBALS['LANG']->csConvObj->isoArray)) $lang = $GLOBALS['LANG']->csConvObj->isoArray[$lang];
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/powermail/pi1/class.tx_powermail_jscalendar.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/powermail/pi1/class.tx_powermail_jscalendar.php']);
}
?>
